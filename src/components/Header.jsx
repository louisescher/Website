...
export default function Header({ text, sub, moreInfoHref }) {
  const headerRef = useRef(null);
  const startAnim = useShowOnScroll(headerRef, 1);

  return (
    <header className="relative">
      <div className="w-px h-px absolute bottom-0 left-0 z-50" ref={headerRef} />
      <div className={`mb-2 w-fit section-header ${startAnim && "active"}`}>
        <h1 className="mb-0.5 large-text relative w-fit">{text}</h1>
