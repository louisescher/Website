...
export default function Header({ text, sub, moreInfoHref, large, small, delay, className }) {
  ...
  return (
    <header className={`relative ${className ? className : ""}`}>
      ...
      <div className={`mb-2 w-fit section-header ${delay && "delay"} ${startAnim && "active"}`}>
        <h1 className={`mb-0.5 mt-0 large-text relative w-fit ${large && "text-5xl mt-4"} ${small && "text-2xl mt-2"}`}>{text}</h1>
